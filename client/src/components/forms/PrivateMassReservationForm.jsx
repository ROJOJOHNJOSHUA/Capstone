...
const purposeFields = {
  "House Blessing": [
    ["family_name", "Name of the Family/Household", "text"],
    ["family_members", "Number of Family Members", "number"],
    ["preferred_time", "Preferred Time", "time"],
  ],
  Thanksgiving: [
    ["person_family_name", "Name of Person/Family", "text"],
    ["preferred_time", "Preferred Time", "time"],
  ],
  "Memorial / Death Anniversary": [
    ["deceased_name", "Name of Deceased", "text"],
    ["date_of_death", "Date of Death", "date"],
    ["relationship", "Relationship to the Requester", "text"],
    ["preferred_time", "Preferred Time", "time"],
  ],
  "Family Gathering": [
    ["person_family_name", "Name of Person/Family", "text"],
...
const locationFields = [
  ["house_block_lot", "House / Block / Lot", "text"],
...
const privateMassMinimumDate = () => {
  const minimum = new Date(`${today()}T12:00:00`);
  minimum.setDate(minimum.getDate() + 7);
  return minimum.toLocaleDateString("en-CA");
};
...
export default function PrivateMassReservationForm({
  user,
  onClose,
  onSubmitted,
  onServiceChange,
  ...
  const minimumDate = privateMassMinimumDate();

  const activeRequirements = useMemo(() => requirements, [requirements]);
  useEffect(() => {
    getDocumentRequirements().then((response) =>
      setRequirements(response.data["Private Mass"] || []),
    );
  }, []);
  useEffect(() => {
    checkMonthlyAvailability(month, "Private Mass")
  ...
  useEffect(() => {
    ...
    checkAvailability(date, "Private Mass")
  ...
  const validate = () => {
    if (
      step === 1 &&
      (!details.fullname.trim() || !details.contact_number.trim())
    )
      return "Full Name and Contact Number are required.";
    if (step === 2) {
      if (!details.purpose)
        return "Please select the purpose of the Private Mass.";
      const required = (purposeFields[details.purpose] || []).filter(
        ([, label]) => !label.includes("Optional"),
      );
      if (required.some(([key]) => !String(purposeDetails[key] || "").trim()))
        return "Please complete all required purpose details.";
    }
    if (step === 3) {
      if (!details.location_type)
        return "Please select the Private Mass location.";
      if (
        locationFields
          .filter(([, label]) => !label.includes("Optional"))
          .some(([key]) => !String(locationDetails[key] || "").trim()) ||
        !details.location_contact_name.trim() ||
        !details.location_contact_number.trim()
      )
        return "Please complete all required location and contact details.";
    }
    if (step === 4 && (!date || date < minimumDate || !time))
      return "Please select a Private Mass date at least one week in advance and choose a time.";
    if (
      step === 4 &&
      !slots.some((slot) => slot.time === time && slot.status === "available")
    )
      return "Please select an available Private Mass time.";
    if (step === 4 || step === 5) {
  ...
  const submit = async (event) => {
    ...
    const message = [1, 2, 3, 4]
      .map((stepToValidate) => validate(stepToValidate))
      .find(Boolean);
    if (message) {
      setError(message);
      return;
    ...
    try {
      ...
      setError(
        requestError.message ||
          "Unable to create the Private Mass reservation. Please try again later.",
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="p-5 sm:p-6">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-[#f8fafc] p-4 sm:p-5">
            ...
            <div className="mt-4 rounded-2xl border border-[#f2e4bb] bg-[#fffaf0] p-3 text-sm text-slate-700">
              <div>
                ...
                {SERVICE_SCHEDULE["Private Mass"]}
              </div>
              <div className="mt-1">
                <span className="font-semibold text-[#0f2337]">
                  Requirements:
                </span>{" "}
                {SERVICE_REQUIREMENTS["Private Mass"]}
              </div>
            </div>
          </div>
        )}
        {step === 1 && (
          <div className="grid gap-4 sm:grid-cols-2">
            {input("fullname", "Full Name", "text", details.fullname, (value) =>
        ...
        {step === 2 && (
          <div>
            <label className="block text-sm font-medium text-slate-700">
              What is the purpose of the Private Mass?{" "}
              <span className="text-red-600">*</span>
              <select
            ...
            {details.purpose && (
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {purposeFields[details.purpose].map(([key, label, type]) => (
                  <div key={key}>
                    {input(key, label, type, purposeDetails[key], (value) =>
                      setPurposeDetails((current) => ({
                        ...current,
                        [key]: value,
                      })),
                    )}
        ...
        {step === 3 && (
          <div>
            <label className="block text-sm font-medium text-slate-700">
              Where will the Private Mass be held?{" "}
              <span className="text-red-600">*</span>
              <select
                className="input-field mt-2"
                value={details.location_type}
                onChange={(event) =>
                  update("location_type", event.target.value)
                }
              >
                <option value="">Select...</option>
            ...
            {details.location_type && (
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                {locationFields.map(([key, label, type]) => (
                  <div key={key}>
                    {input(key, label, type, locationDetails[key], (value) =>
                ...
                {input(
                  "location_contact_name",
                  "Contact Person at Location - Name",
                  "text",
                  details.location_contact_name,
                  (value) => update("location_contact_name", value),
                )}
                {input(
                  "location_contact_number",
                  "Contact Person at Location - Contact Number",
                  "tel",
                  details.location_contact_number,
                  (value) => update("location_contact_number", value),
                )}
              </div>
            )}
          </div>
          ...
          <div>
            <div className="rounded-[24px] border border-slate-200 bg-[#f8fafc] p-4 sm:p-5">
              ...
              <p className="mb-3 text-xs text-slate-600">
                Private Mass requests must be scheduled at least one week in advance.
                Earliest available date: {formatDate(minimumDate)}.
              </p>
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {cells.map((value, index) => {
                    ...
                    const tooSoon = value && value < minimumDate;
                    const selected = date === value;
                    return value ? (
                      <button
                        type="button"
                        key={value}
                        disabled={!available || tooSoon}
                        className={`h-11 rounded-xl border text-xs font-semibold ${available && !tooSoon ? "border-emerald-300 bg-emerald-100 text-emerald-800" : "border-slate-200 bg-slate-100 text-slate-400"} ${selected ? "ring-2 ring-[#0f2337] ring-offset-1" : ""}`}
                        onClick={() => {
                          setDate(value);
                          setTime("");
                        }}
                  ...
                  {slots.map((slot) => {
                    ...
                    return (
                      <button
                        ...
                        key={slot.time}
                        disabled={!available}
                        className={`rounded-xl border px-4 py-3 text-left text-sm font-medium ${available ? `border-emerald-300 bg-emerald-50 text-emerald-800 ${selected ? "ring-2 ring-[#0f2337]" : ""}` : "border-red-200 bg-red-50 text-red-700"}`}
