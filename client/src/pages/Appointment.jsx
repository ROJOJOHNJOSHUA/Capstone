...
const APPOINTMENT_REVIEW_STEP = APPOINTMENT_STEPS.length - 1;
...
export default function Appointment() {
  ...
  const canContinue = () => {
    ...
    if (currentStep === 2) {
      return form.appointment_date && form.appointment_time && slotItems.some((slot) => slot.time === form.appointment_time && slot.status === 'available');
    }
    return true;
  };
  ...
  const nextStep = () => {
    ...
    setCurrentStep((prev) => Math.min(prev + 1, APPOINTMENT_REVIEW_STEP));
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (currentStep !== APPOINTMENT_REVIEW_STEP) return;
    ...
    setError('');
    if (!form.appointment_date || !form.appointment_time || !slotItems.some((slot) => slot.time === form.appointment_time && slot.status === 'available')) {
    ...
    <DashboardLayout>
      ...
      <div className="overflow-hidden rounded-[26px] border border-[#ece4d3] bg-[#fffdf8] shadow-[0_14px_30px_rgba(83,65,34,0.06)]">
        <div className="flex items-center justify-between px-5 pt-5">
          <h2 className="font-display text-lg font-semibold text-[#2f2a22]">Appointment record</h2>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {appointments.map((a) => (
                <tr key={a.id} className="border-b border-[#f5efe2] align-top transition duration-200 hover:bg-[#faf5ea]">
                  <td className="px-5 py-4 text-[#5b5344]">{a.appointment_date}</td>
                  <td className="px-5 py-4 text-[#5b5344]">{formatSlotTime(a.appointment_time || '')}</td>
                  <td className="px-5 py-4 font-medium text-[#2f2a22]">{a.purpose}</td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      {canCancelAppointment(a.status) && (
                        <button
                          type="button"
                          className="text-sm font-semibold text-red-500 transition hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <Modal isOpen={Boolean(viewing)} onClose={() => setViewing(null)} title="Appointment Details" size="lg" backdropClassName="bg-[#14212b]/45">
        {viewing && (
          <div className="space-y-4 text-sm">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl bg-[#faf5ea] p-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9a8666]">Purpose</p>
                <p className="mt-1 font-medium text-[#2f2a22]">{viewing.purpose}</p>
              </div>
              <div className="rounded-xl bg-[#faf5ea] p-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9a8666]">Status</p>
                <p className="mt-1"><StatusBadge status={viewing.status === 'Pending' ? 'Under Review' : viewing.status} /></p>
              </div>
              <div className="rounded-xl bg-[#faf5ea] p-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9a8666]">Date</p>
                <p className="mt-1 font-medium text-[#2f2a22]">{viewing.appointment_date}</p>
              </div>
              <div className="rounded-xl bg-[#faf5ea] p-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9a8666]">Time</p>
                <p className="mt-1 font-medium text-[#2f2a22]">{formatSlotTime(viewing.appointment_time || '')}</p>
              </div>
            </div>
