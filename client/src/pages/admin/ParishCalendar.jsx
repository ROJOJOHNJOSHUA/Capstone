...
export default function ParishCalendar() {
  ...
  return <DashboardLayout>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <aside className="rounded-xl border border-[#e7dfd2] bg-[#fffdf8] p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><h2 className="font-display text-lg text-[#273746]">Upcoming Events</h2><button type="button" className="text-xs font-semibold text-[#a6813f] transition hover:text-[#7d5d25]" onClick={() => setShowUpcoming(true)}>View All <span aria-hidden="true">→</span></button></div>{upcoming.length ? <div className="space-y-3">{upcoming.map((event) => <div key={event.id} className="flex items-center gap-3 border-b border-[#eee7db] pb-3 last:border-0"><div className="w-11 shrink-0 rounded-lg bg-[#f5efe3] p-2 text-center"><p className="text-[9px] uppercase text-[#a6813f]">{formatDate(event.date, { month: 'short' })}</p><p className="font-display text-lg text-[#273746]">{Number(event.date.slice(-2))}</p></div><div className="min-w-0 flex-1"><button type="button" className="text-left text-xs font-semibold text-[#273746] hover:text-[#a6813f]" onClick={() => setSelectedEvent(event)}>{event.type}</button><p className="text-[10px] text-[#7a7d7f]">{formatTime(event.time)} · {event.name}</p></div><span className="rounded-full bg-[#f5ead5] px-2 py-1 text-[9px] text-[#a6813f]">{event.kind === 'appointment' ? 'Appointment' : 'Reservation'}</span></div>)}</div> : <div className="rounded-xl border border-dashed border-[#d8ccb9] p-5 text-center"><p className="font-semibold text-[#58616a]">No Upcoming Schedules</p><p className="mt-1 text-sm text-[#7a7d7f]">There are currently no approved schedules.</p></div>}</aside>
    </div>
    <Modal isOpen={showUpcoming} onClose={() => setShowUpcoming(false)} title="All Upcoming Schedules" size="lg">
      <div className="space-y-3">
        {upcoming.length ? upcoming.map((event) => (
          <button key={event.id} type="button" onClick={() => { setShowUpcoming(false); setSelectedEvent(event); }} className="flex w-full items-center gap-3 rounded-xl border border-[#e7dfd2] bg-[#fffdf8] p-3 text-left transition hover:border-[#b18a45] hover:bg-[#faf5e9] sm:p-4">
            <div className="w-12 shrink-0 rounded-lg bg-[#f5efe3] p-2 text-center"><p className="text-[9px] uppercase text-[#a6813f]">{formatDate(event.date, { month: 'short' })}</p><p className="font-display text-lg text-[#273746]">{Number(event.date.slice(-2))}</p></div>
            <div className="min-w-0 flex-1"><p className="font-semibold text-[#273746]">{event.type}</p><p className="mt-1 break-words text-xs text-[#7a7d7f]">{formatDate(event.date)} · {formatTime(event.time)} · {event.name}</p></div>
            <span className="hidden shrink-0 rounded-full bg-[#f5ead5] px-2 py-1 text-[9px] text-[#a6813f] sm:inline-flex">{event.kind === 'appointment' ? 'Appointment' : 'Reservation'}</span>
          </button>
        )) : <div className="rounded-xl border border-dashed border-[#d8ccb9] p-8 text-center text-sm text-[#7a7d7f]">No upcoming schedules.</div>}
      </div>
    </Modal>
